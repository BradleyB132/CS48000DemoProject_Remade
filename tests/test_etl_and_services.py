...
def test_filter_and_trends():
    ...
    filtered = etl.filter_and_sort(
        data, filters={"line_number": 2}, sort_by="lot_number", ascending=False
    )
...
def test_discrepancy_detection():
    ...
    ship = pd.DataFrame(
        {"lot_number": ["LX"], "production_date": ["2024-01-01"], "destination": ["X"]}
    )
    consolidated = etl.consolidate_data(prod, qual, ship)
    # quality missing for LX should be flagged (AC9)
    assert consolidated.iloc[0]["flag_missing_quality"]


def test_efficiency_and_session_cleanup():
    """AC11, AC12: ensure Database session works and closes properly."""
