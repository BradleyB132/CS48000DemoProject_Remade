...
def logs_to_dataframe(logs: List[Any]) -> pd.DataFrame:
    ...
    for log in logs:
        ...
        rows.append({
            ...
            'destination': getattr(shipping, 'destination', None) if shipping else None,
            'is_shipped': getattr(shipping, 'is_shipped', False) if shipping else False,
...
def apply_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    date_range = st.sidebar.date_input('Production date range', value=(min_date, max_date))
    ...
    # Shipped filter
    shipped = st.sidebar.selectbox('Shipped status', options=['All', 'Shipped', 'Not shipped'])
    if shipped == 'Shipped':
        df = df[df['is_shipped'] == True]
    elif shipped == 'Not shipped':
        df = df[df['is_shipped'] == False]

    # Defective filter
    defective = st.sidebar.selectbox('Defective', options=['All', 'Defective', 'No defects'])
    if defective == 'Defective':
        df = df[df['is_defective'] == True]
    elif defective == 'No defects':
        df = df[df['is_defective'] == False]
...
def main():
    ...
    if not filtered.empty:
        sort_by = st.selectbox('Sort by', options=['production_date', 'line_number', 'defect_count', 'lot_number'], index=0)
    ...
    selected_id = st.selectbox('Select production_log_id to show details', options=[None] + filtered['production_log_id'].tolist())
    if selected_id:
        ...
        if detail:
            ...
            if detail.shipping_manifest:
                st.write('Shipping:', detail.shipping_manifest.destination, 'Shipped:', detail.shipping_manifest.is_shipped)
            inspections = services.list_inspections_for_log(detail.production_log_id)
            st.write('Inspections:')
            for ins in inspections:
                st.write(f"- {ins.defect_type or 'None'} | Severity: {ins.defect_severity or 'N/A'} | Defective: {ins.is_defective}")
